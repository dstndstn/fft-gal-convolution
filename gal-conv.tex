...
\begin{equation}
  I(r) \propto \exp(- k r^{1/n})
\end{equation}
where we can fix $n=1$ (exponential profile), $n=4$ (deVaucouleurs
profile), or fit for $n$ (\sersic\ profile).  The elliptical shape can
be represented in terms of effective radius, axis ratio, and position
angle, or any other ellipse representation.
...
convolved profile in pixel space via a sinc-shift (Lanczos
