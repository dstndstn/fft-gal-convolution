...
\begin{figure}
  \begin{center}
    \begin{tabular}{ccccc}
      %
      %My galaxy
      && My galaxy FT & log(My galaxy FT) & Naive $-$ My \\
      %
      %\includegraphics[height=0.24\textwidth]{lopass-mine-pix}
      &
      %\raisebox{0.11\textwidth}{$\longleftarrow$}
      &
      ...
      \\
      %
      Naive galaxy && Na\"ive galaxy FT & log(Na\"ive galaxy FT) & Naive $-$ My \\
      %
    ...
    \caption{
      ...
      well sampled.  While the Fourier transforms (second column) look
      similar, taking the logarithm (third column) shows that the
      Na\"ive method has significant power far from the origin, while
      my Fourier Space Gaussian Mixture method drops off as expected.
      Taking the difference (fourth column), it is evident that the
      excess power at high frequencies is due to aliasing of power
      outside the band into the band; the solid contours show the
      Gaussian components of this galaxy, with the dashed contours
      showing the wrapped-around (aliased) component.}
  \end{center}
\end{figure}
