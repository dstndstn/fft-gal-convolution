...
% Calabretta & Greisen arxiv number?
...
standard\cite{wcs2} defines a transformation matrix $\CD$ that takes
...
Writing out the matrix multiplication in \eqnref{eq:vpix}, we get
\begin{align}
V_{\textrm{pix}} = \begin{bmatrix}
V_{1,1} & V_{1,2} \\
V_{2,1} & V_{2,2}
\end{bmatrix}
\end{align}
where the matrix is symmetric so that $V_{1,2} = V_{2,1}$ and the terms are
\begin{align}
V_{1,1} & =
%
...
Calabretta,~M.~R. \& Greisen,~E.~W., 2002, \aa, 395, 1077
